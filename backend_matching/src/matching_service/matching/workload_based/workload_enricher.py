...
@dataclass
class WorkloadEnrichedResult:
    occupation_uri: str
    occupation_label: str | None
    hours_worked: float | None


class OccupationWorkloadEnricher:
    """Attach weekly hours worked per occupation (or None if unavailable)."""

    def __init__(
        self,
        esco_db_path: str | Path = Path(__file__).parents[4] / "data" / "esco.duckdb",
        employed_person_csv_path: str | Path = Path(__file__).parents[4] / "data" / "ilostat" / "HOW_TEMP_SEX_OC2_NB_A.csv",
        employee_csv_path: str | Path = Path(__file__).parents[4] / "data" / "ilostat" / "HOW_XEES_SEX_OC2_NB_A.csv",
    ) -> None:
        self.esco_db_path = str(esco_db_path)
        self.employed_person_csv_path = Path(employed_person_csv_path)
        self.employee_csv_path = Path(employee_csv_path)
        self._occupation_to_isco_2digit: dict[str, str] = {}
        self._rows_primary: dict[tuple[str, str, str], dict[int, float]] = {}
        self._rows_fallback: dict[tuple[str, str, str], dict[int, float]] = {}
        self._load_occupation_index()
        self._load_workload_rows()
    ...
    def _load_occupation_index(self) -> None:
        ...
        for occupation_uri, isco_group in rows:
            digits = "".join(ch for ch in str(isco_group).strip() if ch.isdigit())
            if len(digits) >= 2:
    ...
    def _load_workload_rows(self) -> None:
        self._rows_primary = self._load_rows_from_csv(self.employed_person_csv_path)
        self._rows_fallback = self._load_rows_from_csv(self.employee_csv_path)
    ...
    def _lookup(
        ...
        for key in keys:
            value = self._pick_best_year(self._rows_primary.get(key, {}), reference_year)
            if value is not None:
                return value
            value = self._pick_best_year(self._rows_fallback.get(key, {}), reference_year)
            if value is not None:
                return value
        return None
    ...
    ) -> list[WorkloadEnrichedResult]:
        _ = region
        out: list[WorkloadEnrichedResult] = []
...
if __name__ == "__main__":
    ...
    enricher = OccupationWorkloadEnricher()
    for row in enricher.enrich(sample_occupations, country="AFG", sex="female", reference_year=2021):
        print(row)
