...
@dataclass
class SecurityEnrichedResult:
    occupation_uri: str
    occupation_label: str | None
    occupation_unemployment_risk: float | None


class OccupationSecurityEnricher:
    """Attach unemployment-risk proxy per occupation (or None if unavailable)."""

    def __init__(
        self,
        esco_db_path: str | Path = Path(__file__).parents[4] / "data" / "esco.duckdb",
        unemployment_by_former_occ_csv_path: str | Path = Path(__file__).parents[4] / "data" / "ilostat" / "UNE_TUNE_SEX_OCU_NB_A.csv",
    ) -> None:
        self.esco_db_path = str(esco_db_path)
        self.unemployment_by_former_occ_csv_path = Path(unemployment_by_former_occ_csv_path)
        self._occupation_to_isco_major: dict[str, str] = {}
        self._rows: dict[tuple[str, str, str], dict[int, float]] = {}
        self._load_occupation_index()
        self._load_security_rows()
    ...
    def _load_security_rows(self) -> None:
        if not self.unemployment_by_former_occ_csv_path.exists():
            return
        con = duckdb.connect()
        try:
            rows = con.execute(
                ...
                FROM read_csv_auto(
                    '{self.unemployment_by_former_occ_csv_path.as_posix()}',
                    header = true,
                    all_varchar = true,
                    ignore_errors = true
                )
        ...
        for ref_area, sex, classif1, year, obs_value in rows:
            ...
            self._rows.setdefault(key, {})[int(year)] = float(obs_value)
    ...
    def _lookup(
        ...
        major = self._occupation_to_isco_major.get(occupation_uri)
        if not major:
            return None
        country_norm = self._normalize_country(country)
        sex_norm = self._to_iso_sex(sex)
        keys = [(country_norm, sex_norm, f"OCU_ISCO08_{major}")]
        if sex_norm != "SEX_T":
            keys.append((country_norm, "SEX_T", f"OCU_ISCO08_{major}"))
        for key in keys:
            value = self._pick_best_year(self._rows.get(key, {}), reference_year)
    ...
    ) -> list[SecurityEnrichedResult]:
        _ = region
        out: list[SecurityEnrichedResult] = []
        for item in occupations:
            ...
            out.append(
                SecurityEnrichedResult(
                    ...
                    occupation_unemployment_risk=risk,
                )
            )
        return out
...
if __name__ == "__main__":
    ...
    enricher = OccupationSecurityEnricher()
