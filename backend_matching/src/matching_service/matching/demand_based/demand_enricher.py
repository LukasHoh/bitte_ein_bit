...
@dataclass
class DemandEnrichedResult:
    occupation_uri: str
    occupation_label: str | None
    occupation_demand_level: float | None


class OccupationDemandEnricher:
    """Attach employment-demand values per occupation (or None if unavailable)."""

    def __init__(
        self,
        esco_db_path: str | Path = Path(__file__).parents[4] / "data" / "esco.duckdb",
        occupation_level_2_csv_path: str | Path = Path(__file__).parents[4] / "data" / "ilostat" / "EMP_TEMP_SEX_OC2_NB_A.csv",
        occupation_major_csv_path: str | Path = Path(__file__).parents[4] / "data" / "ilostat" / "EMP_TEMP_SEX_OCU_NB_A.csv",
    ) -> None:
        self.esco_db_path = str(esco_db_path)
        self.occupation_level_2_csv_path = Path(occupation_level_2_csv_path)
        self.occupation_major_csv_path = Path(occupation_major_csv_path)
        self._occupation_to_isco_major: dict[str, str] = {}
        self._occupation_to_isco_2digit: dict[str, str] = {}
        self._rows_oc2: dict[tuple[str, str, str], dict[int, float]] = {}
        self._rows_ocu: dict[tuple[str, str, str], dict[int, float]] = {}
        self._load_occupation_index()
        self._load_demand_rows()
    ...
    def _load_occupation_index(self) -> None:
        ...
        for occupation_uri, isco_group in rows:
            ...
            digits = "".join(ch for ch in isco_group_str if ch.isdigit())
            if not digits:
                continue
            self._occupation_to_isco_major[occupation_uri] = digits[0]
            if len(digits) >= 2:
                self._occupation_to_isco_2digit[occupation_uri] = digits[:2]

    def _load_rows_from_csv(self, csv_path: Path) -> dict[tuple[str, str, str], dict[int, float]]:
    ...
    def _load_demand_rows(self) -> None:
        self._rows_oc2 = self._load_rows_from_csv(self.occupation_level_2_csv_path)
        self._rows_ocu = self._load_rows_from_csv(self.occupation_major_csv_path)
    ...
    def _lookup(
        ...
        keys_to_try: list[tuple[dict[tuple[str, str, str], dict[int, float]], tuple[str, str, str]]] = []
        code2 = self._occupation_to_isco_2digit.get(occupation_uri)
        if code2:
            keys_to_try.append((self._rows_oc2, (country_norm, sex_norm, f"OC2_ISCO08_{code2}")))
            if sex_norm != "SEX_T":
                keys_to_try.append((self._rows_oc2, (country_norm, "SEX_T", f"OC2_ISCO08_{code2}")))

        code1 = self._occupation_to_isco_major.get(occupation_uri)
        if code1:
            keys_to_try.append((self._rows_ocu, (country_norm, sex_norm, f"OCU_ISCO08_{code1}")))
            if sex_norm != "SEX_T":
                keys_to_try.append((self._rows_ocu, (country_norm, "SEX_T", f"OCU_ISCO08_{code1}")))

        for source_rows, key in keys_to_try:
            value = self._pick_best_year(source_rows.get(key, {}), reference_year)
    ...
    ) -> list[DemandEnrichedResult]:
        _ = region
        out: list[DemandEnrichedResult] = []
        for item in occupations:
            occupation_uri, occupation_label = self._extract_occupation(item)
            demand_value = self._lookup(
            ...
            out.append(
                DemandEnrichedResult(
                    ...
                    occupation_demand_level=demand_value,
                )
            )
        return out
...
if __name__ == "__main__":
    ...
    enricher = OccupationDemandEnricher()
    for row in enricher.enrich(sample_occupations, country="AFG", sex="female", reference_year=2020):
        print(row)
